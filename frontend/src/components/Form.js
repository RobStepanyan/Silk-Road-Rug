...
export default class Form extends Component {
  ...
  handleInputChange(event, context, i, required, validate, title, onlyText, maxLength) {
    let pwds, confirmPwd, val = event.target.value, valid, file = context === 'file' ? event.target.files[0] : '';
    ...
    if (((validate === false) && val.length > 1 && val.length < 255)
      ...
      || (context === 'file' && ((!required && event.target.length === 0) || file.size / 1024 / 1024 < this.state.fileMaxSizeMB))
