...
export default class Cart extends React.Component {
  ...
  sendPartialUpdate(keyProp) {
    ...
    let selecteds = []
    Object.entries(selectedRadios).forEach(entry => {
      let [key, val] = entry
      selecteds.push(Object.values(cartCardInputsOrder[key])[val])
    })
    Object.entries(selectedCheckboxes).forEach(entry => {
      let [key, val] = entry
      val.forEach(selCheck => {
      ...
      .then(res => {
        ...
        data.forEach((cartItem, i) => {
          Object.entries(types).forEach(entry => {
            ...
            cartItem.selecteds.forEach(sel => {
