...
def rug_post_save_receiver(sender, instance, *args, **kwargs):
    ...
    for x in models.RugVariation.objects.filter(rug=model_to_dict(instance)['rug']):
        ...
        widths.append(x.width_feet)
        heights.append(x.height_feet)
    ...
    m.width_smallest = widths[0]
    m.width_largest = widths[-1]
    m.height_smallest = heights[0]
    m.height_largest = heights[-1]
    m.save()
